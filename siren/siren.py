...
class SPATIALSIRENBASELINE(nn.Module):
    ...
    def __init__(self, input_dim=2, z_dim=100, hidden_dim=256, output_dim=1, shading=False,
                view_condition=True, light_condition=True, device=None):
        super().__init__()
        self.device = device
        self.input_dim = input_dim
        self.z_dim = z_dim
        ...
        if self.view_condition:
            color_input_dim += 3  # view direction
        if self.light_condition:
            color_input_dim += 4  # lighting param
        self.color_layer_sine = FiLMLayer(color_input_dim, hidden_dim)
    ...
    def forward(self, input, z, l, ray_directions, rt_normal=False, **kwargs):
        frequencies, phase_shifts = self.mapping_network(z)
        return self.forward_with_frequencies_phase_shifts(input, frequencies, phase_shifts, l, ray_directions, rt_normal, **kwargs)
    
    def forward_with_frequencies_phase_shifts(self, input, frequencies, phase_shifts, l, ray_directions, rt_normal=False, **kwargs):
        with_grad = torch.is_grad_enabled()
        outputs = {}
        
        with torch.set_grad_enabled(self.shading or rt_normal or with_grad):
            ...
            if self.shading and self.light_condition:
                x = torch.cat([x, l.unsqueeze(1).expand(-1,x.size(1),-1)], dim=-1)

            rgb = self.color_layer_sine(x, frequencies[..., -self.hidden_dim:], phase_shifts[..., -self.hidden_dim:])
